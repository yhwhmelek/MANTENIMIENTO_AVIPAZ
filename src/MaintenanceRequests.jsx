...
export default function MaintenanceRequests({ apiUrl, token, currentUser, open, onOpen, onClose }) {
  ...
  return <>
    ...
    <dialog ref={dialog} className="request-workspace" aria-labelledby="requests-title" onCancel={e=>{if(busy)e.preventDefault()}} onClose={onClose}>
      ...
      {!form&&!row&&showBacklog&&<PrioritizedActivities rows={rows.filter(r=>!filter||r.status===filter)} plants={plants} towers={towers} plantFilter={plantFilter} setPlantFilter={setPlantFilter} statusFilter={filter} apiUrl={apiUrl} token={token} busy={busy} canEdit={canCreate&&catalogReady} isAdmin={isAdmin} onSaved={()=>setVersion(v=>v+1)} onOpen={id=>{setSelected(id);setFormError('')}} onEdit={editRequest}/>}
