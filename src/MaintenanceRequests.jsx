...
export default function MaintenanceRequests({ apiUrl, token, currentUser, open, onOpen, onClose }) {
  ...
  const canCreate=['ADMIN','OPERADOR','MECANICO','ELECTRICO'].includes(currentUser.rol)
  const assignedWorkCount=rows.filter(r=>(r.status==='PENDIENTE'&&Number(r.request_data.planning?.assigned_user_id)===currentUser.id)||(r.status==='EN_PROCESO'&&r.assigned_to===currentUser.id)).length
  const assignedWork=rows.filter(r=>(r.status==='PENDIENTE'&&Number(r.request_data.planning?.assigned_user_id)===currentUser.id)||(r.status==='EN_PROCESO'&&r.assigned_to===currentUser.id))
  const managementCount=isAdmin?rows.filter(r=>r.status==='PENDIENTE'&&!r.request_data.planning?.assigned_user_id).length:0
  ...
  return <>
    <button className={`request-alert-trigger ${assignedWorkCount||managementCount||error?'needs-attention':''}`} onClick={onOpen}><Bell size={18}/><span role="status">{error?'Solicitudes sin verificar':loaded?`Trabajos asignados pendientes: ${assignedWorkCount}${isAdmin?` · Sin asignar: ${managementCount}`:''}`:'Consultando solicitudes…'}</span></button>
    <dialog ref={dialog} className="request-workspace" aria-labelledby="requests-title" onCancel={e=>{if(busy)e.preventDefault()}} onClose={onClose}>
      ...
      {row&&!form&&<section ref={detail} tabIndex={-1} aria-label={`Detalle de solicitud ${row.id}`} className="request-detail"><button type="button" className="secondary-action" disabled={busy} onClick={()=>{setSelected(null);setFormError('')}}>Volver al listado</button>{row.request_data.maintenance_type==='MEJORA_TECNICA'&&<button type='button' className='primary-action' disabled={busy||!canCreate||!catalogReady||row.status!=='PENDIENTE'} title={!canCreate?'Solo el personal de mantenimiento puede modificar':row.status!=='PENDIENTE'?'Solo se modifican solicitudes pendientes':'Modificar solicitud'} onClick={()=>editRequest(row.id)}>Modificar solicitud</button>}<h3>Solicitud #{row.id} · {states[row.status]}</h3><p>{row.request_data.machine_name} · {row.request_data.description}</p><p>Tipo: {row.request_data.maintenance_type}. Falla: {row.request_data.failure?'Sí':'No'}.</p><p>Inicio programado: {time(row.request_data.planning?.starts_at)}. Duración estimada: {row.request_data.planning?planDuration(row.request_data.planning):'Sin programar'}. Parada: {time(row.request_data.stopped_at)}.</p>
        ...
        <div className="request-toolbar"><button className="secondary-action" onClick={()=>setPrintRow(row)}>Imprimir / guardar PDF {row.request_data.maintenance_type==='MEJORA_TECNICA'?'MT/02-08':'MT/02-05'}</button>
          ...
          {((row.status==='PENDIENTE'&&Number(row.request_data.planning?.assigned_user_id)===currentUser.id)||(row.status==='EN_PROCESO'&&row.assigned_to===currentUser.id))&&executionBlock&&<p role="status">No se puede iniciar o entregar todavía: {executionBlock}</p>}
          {row.status==='PENDIENTE'&&Number(row.request_data.planning?.assigned_user_id)===currentUser.id&&<form onSubmit={startWork}><label>Tiempo aproximado de reparación (horas)<input type="number" min="0.02" max="8760" step="0.01" required value={estimatedHours} onChange={event=>setEstimatedHours(event.target.value)}/></label><label>Fecha y hora real de inicio<input type="datetime-local" required value={startedAt} onChange={event=>setStartedAt(event.target.value)}/></label><p>Registra la fecha en que realmente comenzó el trabajo (hora de Ecuador).</p><button disabled={busy||Boolean(executionBlock)} title={executionBlock||'Registrar inicio del trabajo'} className="primary-action">Iniciar trabajo programado</button></form>}
          {row.status==='EN_PROCESO'&&row.assigned_to===currentUser.id&&<button disabled={busy||!catalogReady||!row.priority||!favorable||row.request_data.planning?.condition!=='LISTA'} className="primary-action" onClick={()=>start('complete')}>Registrar trabajo y repuestos</button>}
