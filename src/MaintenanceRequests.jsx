...
export default function MaintenanceRequests({ apiUrl, token, currentUser, open, onOpen, onClose }) {
  ...
  const canExecute=row=>(isContractor(row)&&isAdmin)||Number(row?.request_data.planning?.assigned_user_id)===Number(currentUser.id)||Number(row?.assigned_to)===Number(currentUser.id)
  ...
  function start(modeName,targetRow=row){
    ...
    if(modeName==='complete'){
      ...
      setForm({repair_started_at:row.accepted_at.slice(0,16),repair_finished_at:localInput(),
        ...
        hour_meter:original.hour_meter??'',waiting_parts_minutes:'0',work_done:isTechnician?'':(original.maintenance_type==='MEJORA_TECNICA'?original.improvement_proposal:original.description)||'',cause:'',recommendations:'',delivery_conditions:'',
  ...
  async function save(event){
    ...
    if(mode==='complete'){
      ...
      if(isTechnician){payload.work_done=payload.work_done?.trim();payload.cause=payload.cause?.trim()||null;payload.improvement_result=row.request_data.maintenance_type==='MEJORA_TECNICA'?payload.work_done:'';payload.recommendations=payload.recommendations?.trim()||null;payload.delivery_conditions=payload.delivery_conditions?.trim()||null;payload.parts=[];payload.tools=[]}
  ...
  return <>
    ...
    <dialog ref={dialog} className="request-workspace" aria-labelledby="requests-title" onCancel={e=>{if(e.target!==e.currentTarget||busy)e.preventDefault()}} onClose={e=>{if(e.target===e.currentTarget)onClose()}}>
      ...
      {form&&<form onSubmit={save}><h3>{mode==='new'?'Generar solicitud':mode==='edit'?'Completar mejora técnica':mode==='admin_flow'?'Corregir fechas del flujo':mode==='complete'?'Registrar trabajo realizado':'Confirmar recepción'}</h3><p>Fechas y horas locales de Ecuador (UTC−5).</p><fieldset disabled={busy} className="request-fields"><div className="motor-form-grid">
        ...
        {mode==='complete'&&isTechnician&&<><p className="full-field">El inicio ya fue registrado al comenzar el trabajo. Al entregar se guardará automáticamente la hora de finalización.</p><Text label="Trabajo realizado" name="work_done" maxLength={2000} form={form} change={change}/><Text label="Posibles causas" name="cause" maxLength={1000} required={false} form={form} change={change}/><Text label="Recomendaciones de cómo operar la máquina" name="recommendations" maxLength={1000} required={false} form={form} change={change}/><Text label="Condiciones de entrega" name="delivery_conditions" maxLength={1000} required={false} form={form} change={change}/><ImageAttachment label="Foto del trabajo realizado (opcional)" onChange={event=>setPhoto(event.target.files?.[0]||null)} help="La imagen se comprimirá y guardará como evidencia. JPG, PNG o WEBP; máximo 10 MB."/></>}
