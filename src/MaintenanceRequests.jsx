...
export default function MaintenanceRequests({ apiUrl, token, currentUser, open, onOpen, onClose }) {
  ...
  return <>
    ...
    <dialog ref={dialog} className="request-workspace" aria-labelledby="requests-title" onCancel={e=>{if(busy)e.preventDefault()}} onClose={onClose}>
      ...
      {!form&&!row&&showBacklog&&<PrioritizedActivities rows={rows.filter(r=>!plantFilter||String(r.request_data.plant_id)===plantFilter)} busy={busy} canEdit={canCreate&&catalogReady} onOpen={id=>{setSelected(id);setFormError('')}} onEdit={editRequest}/>}
