...
export default function PrioritizedActivities({rows,plants,towers,plantFilter,setPlantFilter,statusFilter,apiUrl,token,onOpen,onEdit,canEdit,isAdmin,onSaved,busy}){
  ...
  function closePlanning(){
    if(planningDialog.current?.open)planningDialog.current.close()
    setPlanningRow(null)
  }
  function planningSaved(){
    closePlanning()
    onSaved?.()
  }
  ...
  return <section><h3>Lista única de actividades priorizadas</h3>
    <p>Orden: primero las actividades por programar y después las programadas. Dentro de cada grupo se conserva el orden de prioridad: solicitudes sin validar primero y luego prioridad final de mayor a menor.</p>
    ...
    {createPortal(<dialog ref={planningDialog} className="request-workspace" aria-labelledby="planning-dialog-title" onCancel={event=>{event.preventDefault();event.stopPropagation();closePlanning()}} onClose={event=>{event.stopPropagation();setPlanningRow(null)}}>{planningRow&&<><div className="modal-header"><h2 id="planning-dialog-title">{planningRow.request_data.planning?'Editar programación':'Programar actividad'} · Solicitud #{planningRow.id}</h2><button type="button" aria-label="Cerrar programación" onClick={closePlanning}>Cerrar</button></div><p>Cambia la fecha de inicio, el tiempo estimado y el responsable antes de iniciar el trabajo.</p><PriorityWorkflow key={planningRow.id} row={planningRow} isAdmin={isAdmin} request={request} initialMode="plan" compact onCancel={closePlanning} onSaved={planningSaved}/></>}</dialog>,document.body)}
  </section>
}
