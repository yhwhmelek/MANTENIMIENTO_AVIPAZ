...
export default function PrioritizedActivities({rows,onOpen,busy}){
  ...
  function exportList(){
    ...
    const data=[['Solicitud','Fecha','Tipo','Planta','Torre','Equipo / area','Actividad','N solicitante','I solicitante','C solicitante','N validado','I validado','C validado','PR','Nivel base','Prioridad final','Escalamiento','Estado','Responsable','Recursos','Permisos','Ventana','Inicio programado','Fin programado','Condicion','Observaciones'],...sorted.map(row=>{
      const r=row.request_data,p=r.preevaluation||{},v=r.priority_validation?.factors||{},plan=r.planning||{}
      return [row.id,row.requested_at,r.maintenance_type,r.plant_name,r.tower_name,r.target_area||r.machine_name,r.description,p.n,p.i,p.c,v.n,v.i,v.c,row.priority?.score,levels[row.priority?.base],levels[row.priority?.level]||'Sin validar',row.priority?.escalated?'C=4: minimo ALTO':'',row.status,plan.responsible,plan.resources,plan.permits,plan.window,plan.starts_at,plan.ends_at,conditions[plan.condition],plan.notes]
