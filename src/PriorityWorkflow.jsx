...
export default function PriorityWorkflow({row,isAdmin,request,onSaved}){
  ...
  async function start(kind){
    setError('')
    if(kind==='plan'&&!users.length){
      setBusy(true)
      try{setUsers((await request('/usuarios')).filter(user=>user.activo))}catch(err){setError(err.message);setBusy(false);return}
      setBusy(false)
    }
    setMode(kind)
