...
export default function Dashboard({ apiUrl, token, currentUser, onUserChange, onLogout }) {
  ...
  useEffect(() => {
    if (section !== 'spare-parts') return
    const controller = new AbortController()
    let pending = false
    setWarehouseStock(null); setStockError('')
    async function refreshStock() {
      if (pending || controller.signal.aborted) return
      pending = true
      try {
        const response = await fetch(`${apiUrl}/stock-repuestos`, { headers: { Authorization: `Bearer ${token}` }, signal: controller.signal })
        const data = await response.json()
        if (!response.ok) throw new Error(requestError(data, 'No se pudo consultar el stock de bodega.'))
        setWarehouseStock(Object.fromEntries(data.map(item => [item.spare_part_id, item.current_stock])))
        setStockError('')
      } catch (err) {
        if (err.name !== 'AbortError') { setStockError(err.message); setWarehouseStock(null) }
      } finally { pending = false }
    }
    refreshStock()
    const timer = setInterval(refreshStock, 30000)
    window.addEventListener('stock-updated', refreshStock)
    window.addEventListener('focus', refreshStock)
    return () => { controller.abort(); clearInterval(timer); window.removeEventListener('stock-updated', refreshStock); window.removeEventListener('focus', refreshStock) }
  }, [apiUrl, token, section, spareParts])
  ...
  async function deleteUser(user) {
    const name=user.nombre_completo||user.nombre
    if(user.id===currentUser.id)return setMessage('No puedes eliminar tu propia cuenta mientras tienes la sesión iniciada.')
    if(!window.confirm(`¿Eliminar definitivamente al usuario ${name}? Esta acción solo será posible si no tiene historial asociado.`))return
    setLoading(true);setMessage('')
    try{
      const response=await fetch(`${apiUrl}/usuarios/${user.id}`,{method:'DELETE',headers:authHeaders})
      const data=await response.json().catch(()=>({}))
      if(!response.ok)throw new Error(requestError(data,'No se pudo eliminar el usuario.'))
      setUsers(items=>items.filter(item=>item.id!==user.id))
      setEditingUser(current=>current?.id===user.id?null:current)
      setMessage(`Usuario ${name} eliminado correctamente.`)
    }catch(error){setMessage(error.message)}finally{setLoading(false)}
  }
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-content">
      ...
      {isSparePartsSection && <nav className="spare-parts-nav" aria-label="Repuestos">
        ...
        <button aria-current={section === 'requisitions' ? 'page' : undefined} onClick={() => { setMessage(''); setSection('requisitions') }}>Requisiciones de compra</button>
