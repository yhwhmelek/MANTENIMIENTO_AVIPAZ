...
const dataLabels = { NONE: 'Sin data', MOTOR: 'Data de motor', REDUCTOR: 'Data de reductor' }
...
export default function MachineElementTypes({ apiUrl, token, isAdmin }) {
  ...
  async function save(event) {
    ...
    const payload = { specification_type: form.get('specification_type'), name: form.get('name').trim(), description: form.get('description').trim() || null, active: form.get('active') === 'on' }
  ...
  return <>
    ...
    <div className="users-card table-scroll"><table><thead><tr>{isAdmin && <th>Acciones</th>}<th>Nombre</th><th>Tipo de data</th><th>Descripción</th><th>Estado</th></tr></thead><tbody>{items.map((item) => <tr key={item.element_type_id}>
      ...
      <td><strong>{item.name}</strong></td><td>{dataLabels[item.specification_type] || dataLabels.NONE}</td><td>{item.description || '—'}</td><td><span className={`status-badge ${item.active ? 'active' : 'inactive'}`}>{item.active ? 'Activo' : 'Inactivo'}</span></td>
    ...
    {showForm && isAdmin && <div className="modal-backdrop"><div className="motor-modal category-modal" role="dialog" aria-modal="true" aria-labelledby="element-type-title">
      ...
      <form onSubmit={save}><div className="motor-form-grid category-form-grid">
        ...
        <label>Tipo de data<select name="specification_type" defaultValue={editing?.specification_type || 'NONE'}>{Object.entries(dataLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><span className="field-help">Todos los elementos de este tipo usan la data seleccionada.</span></label>
