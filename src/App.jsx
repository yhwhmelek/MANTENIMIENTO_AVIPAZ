...
function App() {
  ...
  async function handleRegister(event) {
    event.preventDefault()
    const form = new FormData(event.currentTarget)
    const nombre = form.get('nombre')?.trim(), nombres = form.get('nombres')?.trim(), apellidos = form.get('apellidos')?.trim(), correo = form.get('correo')?.trim(), password = form.get('password'), confirmar = form.get('confirmarPassword')
    if (!nombre || !nombres || !apellidos || !correo || !password || !confirmar) return setMessage('Completa todos los campos.')
    ...
    setLoading(true); setMessage('')
    try {
      const response = await fetch(`${API_URL}/auth/registro`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nombre, nombres, apellidos, correo, password }) })
      const data = await response.json().catch(() => ({}))
      ...
      event.currentTarget.reset(); setView('login'); setMessage(`Cuenta creada para ${data.nombre_completo}. Ya puedes iniciar sesion.`)
    } catch (error) { setMessage(error instanceof TypeError ? 'No se pudo conectar con el servidor.' : error.message) } finally { setLoading(false) }
  }
