...
export function compareActivities(a,b){
  ...
  const validatedA=Boolean(a.request_data.priority_validation?.factors)
  const validatedB=Boolean(b.request_data.priority_validation?.factors)
  return Number(validatedA)-Number(validatedB)
    ||(validatedA ? (rank[a.priority?.level]??4)-(rank[b.priority?.level]??4) : 0)
